...
static int read_dns_name(uint8_t *name, unsigned int name_len, uint8_t *buf, uint8_t *resp) {
...
static void receive_cf_response(void *buffer, size_t size, size_t nmemb, void *userp) {
    // printf("Received response: %s\n", buffer);
    if (strstr((char *)buffer, "\"success\":true") == NULL) {
        is_dirty = true;
        syslog(LOG_ERR, "Cloudflare API request failed (Zone %s, DNS Record %s): %s", _zone_id, _record_id, (char *)buffer);
    }
    log_debug("Cloudflare update successful (Zone %s, DNS Record %s)", _zone_id, _record_id);
}
...
static int update_dns_record(char *zone_id, char *record_id) {
    ...
    snprintf(auth_header, sizeof(auth_header), "Authorization: Bearer %s", cfg_cf_token);
    struct curl_slist *headers = curl_slist_append(NULL, auth_header);
    headers = curl_slist_append(headers, "Content-Type: application/json");

    char json_payload[512];
    snprintf(json_payload, sizeof(json_payload), "{\"content\":\"%s\"}", inet_ntoa(addr));

    log_debug("Sending request to %s, payload: %s, auth: %s\n", url, json_payload, auth_header);
...
static int parse_config(void) {
    ...
    while (1) {
        ...
        if (!strcmp("cf_token", key)) {
            ...
            cfg_zones_len += 1;
            if (cfg_zones != NULL) {
                cfg_zones = realloc(cfg_zones, cfg_zones_len * sizeof(struct cfg_dns_zone));
            } else {
...
static void init(void) {
    ...
    ret = dns_init();
    if (ret < 0) {
        syslog(LOG_ERR, "Could not open UDP socket for DNS Queries: %s.", strerror(ret));
        deinit();
        exit(1);
    }
...
static void on_sigint_sigterm(int signum) {
    syslog(LOG_INFO, "Caught signal %s, exiting...", signum == SIGINT ? "SIGINT" : "SIGTERM");
    deinit();
    exit(0);
}
